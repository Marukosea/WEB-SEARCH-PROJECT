...
if (keyFilename && fs.existsSync(keyFilename)) {
    // Jika file JSON ada, baca dari file
    try {
        config = JSON.parse(fs.readFileSync(keyFilename, 'utf8'));
    ...
    // Jika file tidak ada, baca dari environment variable
    try {
        config = JSON.parse(process.env.GOOGLE_CREDENTIALS_JSON);
        console.log('Config loaded from environment variable successfully:', config.project_id);
    } catch (err) {
        console.error('Error parsing GOOGLE_CREDENTIALS_JSON:', err.message);
        process.exit(1); // Keluar jika parsing gagal
    }
} else {
    console.error('Error: GOOGLE_APPLICATION_CREDENTIALS atau GOOGLE_CREDENTIALS_JSON tidak ditemukan');
    process.exit(1); // Keluar jika tidak ada konfigurasi
}

const storage = new Storage({ credentials: config });
